...
function updateUI(user, data) {
    ...
    // ✅ حفظ البريد الإلكتروني في localStorage لاستخدامه في subscribe.html
    if (user && user.email) {
...
if (!user) {
    ...
    // ✅ إظهار زر إنشاء حساب للمستخدم غير المسجل (مع إزالة أي تأثير من CSS)
    if (navSignupBtn) {
 ...
 // حالة مستخدم مسجل
    ...
    // ✅ إخفاء زر إنشاء حساب للمستخدم المسجل (بشكل قطعي)
    if (navSignupBtn) {
    ...
    _currentUserStatus = isPremium ? 'premium' : 'free';
            ...
            if (isPremium) {
                ...
                // ✅ تحديث حالة المراجعة المميزة في القائمة الجديدة
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // فتح الإعدادات من أيقونة الترس في الملف الشخصي
    ...
    // ===== ربط صف "لديك" بنافذة تعديل المعلومات =====
    ...
    if (saveEditInfoBtn) {
        saveEditInfoBtn.addEventListener('click', function() {
            ...
            if (!date) { alert('يرجى اختيار تاريخ الامتحان.'); return; }
            ...
            // إغلاق النافذة وإعادة فتح القائمة؟
            editInfoModal.style.display = 'none';
        });
    }
