// ============================================
// exams.js - نظام الامتحانات المتكامل (نسخة مبسطة)
// ============================================
...
async function getUserStatusForExam() {
  ...
  let now = Date.now();
  if (examUserStatusCache && (now - examLastStatusCheck) < 60000) return examUserStatusCache;
  try {
...
function renderTeileList() {
  ...
  for (let i = 0; i < teile.length; i++) {
    ...
    btn.style.cssText = `
      ...
      white-space: nowrap;
    `;
    btn.onmouseenter = () => { if (!isActive) { btn.style.background = '#202534'; btn.style.color = '#FFFFFF'; } };
    btn.onmouseleave = () => { if (!isActive) { btn.style.background = '#161922'; btn.style.color = '#BFC6D4'; } };
...
function renderMündlichPartTabs() {
  ...
  parts.forEach(part => {
    ...
    btn.style.cssText = `background: ${currentMündlichPart === part.id ? "#4a6fa5" : "#eef2f7"}; color: ${currentMündlichPart === part.id ? "white" : "#2c3e66"}; border: none; padding: 8px 20px; border-radius: 30px; cursor: pointer; font-size: 14px; font-weight: 500; transition: all 0.2s;`;
    btn.onmouseenter = () => { if (currentMündlichPart !== part.id) { btn.style.background = "#dee2e8"; } };
    btn.onmouseleave = () => { if (currentMündlichPart !== part.id) { btn.style.background = "#eef2f7"; } };
    btn.onclick = () => { currentMündlichPart = part.id; renderExamListForSkill(part.skill, `Mündlich - ${part.name}`); };
...
// ========== ✅ الدالة الأساسية: فتح الامتحان (معدلة) ==========
async function openExam(examId, examTitle, skill) {
  ...
  try {
    ...
    const hasQuestions = currentExamData.questions && Array.isArray(currentExamData.questions) && currentExamData.questions.length > 0;
    ...
    if (currentExamData.type === "matching") {
      if (typeof window.loadMatchingExam === "function") {
        ...
        showNoQuestionsMessage("teil1", currentExamData.title);
      }
      ...
      if (container && typeof window.buildTrueFalseExam === "function") {
        ...
        showNoQuestionsMessage(currentSkill, currentExamData.title);
      }
      ...
      if (typeof window.loadTeil2Exam === "function") {
        ...
        showNoQuestionsMessage("teil2", currentExamData.title);
      }
      ...
      if (typeof window.loadTeil3Exam === "function") {
        ...
        showNoQuestionsMessage("teil3", currentExamData.title);
      }
      ...
      if (typeof window.loadSprach1Exam === "function") {
        ...
        showNoQuestionsMessage("sprach1", currentExamData.title);
      }
      ...
      if (typeof window.loadSprach2Exam === "function") {
        ...
        showNoQuestionsMessage("sprach2", currentExamData.title);
      }
      ...
      if (typeof window.loadSchreibenExam === "function") {
        ...
        showNoQuestionsMessage("schreiben", currentExamData.title);
      }
...
// ========== ✅ دالة عرض رسالة "لا توجد أسئلة" ==========
function showNoQuestionsMessage(containerId, title) {
  ...
  if (container) {
    container.innerHTML = `<div style="text-align:center;padding:40px;color:#999;font-size:1.1rem;">
      ...
      <small style="color:#bbb;">${title}</small>
    </div>`;
  }
}
...
function renderInfoExam(examData) {
  ...
  const container = document.getElementById(containerId);
  if (!container) { console.error("❌ الحاوية غير موجودة:", containerId); return; }
  container.innerHTML = "";
  const content = examData.content;
  if (!content) { container.innerHTML = "<div class='error'>⚠️ لا يوجد محتوى للعرض</div>"; return; }
  let html = `<div style="max-width: 1300px; margin: 0 auto; padding: 20px;">
  ...
  if (content.phase1) {
    ...
    content.phase1.questions.forEach(q => {
      html += `<div style="background: #ffffff; border-radius: 16px; padding: 18px; border: 1px solid #e8ecef;">
  ...
  if (content.phase2) {
    ...
    content.phase2.topics.forEach(topic => {
      html += `<div style="background: #f8f9fb; border-radius: 16px; padding: 18px; border: 1px solid #e8ecef;">
  ...
  if (content.groups) {
    ...
    content.groups.forEach(group => {
      html += `<div style="background: #f8f9fb; border-radius: 16px; padding: 20px; border: 1px solid #e8ecef; display: flex; flex-direction: column;">
    ...
    if (content.methodology) {
      html += `<div style="background: #f8f9fb; border-radius: 16px; padding: 20px; border: 1px solid #e8ecef;">
...
function renderTipsExam(examData) {
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    card.style.cssText = `background: #f8f9fa; border-radius: 16px; padding: 20px; margin-bottom: 20px; border-right: 4px solid #28a745; border-left: 1px solid #e0e0e0; border-top: 1px solid #e0e0e0; border-bottom: 1px solid #e0e0e0; box-shadow: 0 2px 8px rgba(0,0,0,0.05); font-size: 16px; line-height: 1.7; color: #333; white-space: pre-wrap;`;
    let formattedText = p;
...
function createMündlichCard(title, text) {
  const card = document.createElement("div");
  card.style.cssText = `background: #f8f9fa; border-radius: 16px; padding: 20px; margin-bottom: 20px; border: 1px solid #e0e0e0; box-shadow: 0 2px 8px rgba(0,0,0,0.05);`;
