...
// ============================================
// التحكم في أزرار الشريط الجانبي حسب المهارة
// ============================================
function updateSidebarButtons() {
    ...
    const rapidGameBtn = document.getElementById('rapidGameBtn');
    const memoryToggleBtn = document.getElementById('memoryToggleBtn');
    
    // المهارات المدعومة (تظهر فيها كل الأزرار)
    const fullSupported = ['hoeren1', 'hoeren2', 'hoeren3', 'lesen1', 'lesen2', 'lesen3'];
    
    // المهارات التي يظهر فيها كل شيء ما عدا زر التخليط
    const noInterleaving = ['sprach1', 'sprach2'];
    
    // المهارات التي لا يظهر فيها شيء
    const hiddenAll = ['mündlich1', 'mündlich2', 'mündlich3', 'schreiben'];
    
    // إخفاء الكل افتراضياً
    if (interleavingBtn) interleavingBtn.style.display = 'none';
    if (rapidGameBtn) rapidGameBtn.style.display = 'none';
    if (memoryToggleBtn) memoryToggleBtn.style.display = 'none';
    
    // إذا كانت المهارة مدعومة بالكامل (كل الأزرار)
    if (fullSupported.includes(skill)) {
        if (interleavingBtn) interleavingBtn.style.display = 'flex';
        if (rapidGameBtn) rapidGameBtn.style.display = 'flex';
        if (memoryToggleBtn) memoryToggleBtn.style.display = 'flex';
    }
    // إذا كانت المهارة مدعومة جزئياً (بدون زر التخليط)
    else if (noInterleaving.includes(skill)) {
        if (rapidGameBtn) rapidGameBtn.style.display = 'flex';
        if (memoryToggleBtn) memoryToggleBtn.style.display = 'flex';
        // interleavingBtn يبقى مخفياً
    }
    // المهارات غير المدعومة (كل شيء مخفي)
    else if (hiddenAll.includes(skill)) {
        // كل شيء مخفي بالفعل
    }
}

// استدعاء الدالة عند تغيير الامتحان
function onExamChange(skill) {
    window.currentSkill = skill;
    setTimeout(updateSidebarButtons, 50);
}

// ربط الدالة بتغيير الامتحان
const originalOpenExam = window.openExam;
if (originalOpenExam) {
    window.openExam = async function(examId, examTitle, skill) {
        await originalOpenExam(examId, examTitle, skill);
        setTimeout(updateSidebarButtons, 100);
    };
}

// استدعاء الدالة عند تحميل الصفحة
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(updateSidebarButtons, 300);
});
...
console.log('✅ نظام التحكم في الأزرار الجانبية جاهز');
