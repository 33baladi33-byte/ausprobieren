...
// يدعم: Matching + True/False + Teil 2 + Teil 3 + Sprachbausteine Teil 1 + Sprachbausteine Teil 2 + Schreiben + التلوين الذكي
// ============================================

console.log("✅ engine.js تم تحميله (النسخة النهائية)");

// ============================================
// نظام التلوين (Coloring System)
// ============================================

let coloringActive = true;

// دالة تطبيق التلوين على النصوص
function applyColoring() {
    const examData = window.currentExamData || getCurrentExamData();
    if (!examData) return;
    
    const questions = examData.questions || examData.items || [];
    questions.forEach((q, index) => {
        if (q.highlight) {
            colorizeQuestion(q, index);
        }
    });
}
...
// دالة تبديل حالة التلوين
function toggleColoring() {
    if (coloringActive) {
        removeColoring();
        coloringActive = false;
        updateColoringButton('🎨 عرض التلوين');
    } else {
        applyColoring();
        coloringActive = true;
        updateColoringButton('🚫 إخفاء التلوين');
    }
}

function updateColoringButton(text) {
    const btn = document.getElementById('coloringToggleBtn');
    if (btn) btn.textContent = text;
}

// دالة تلوين سؤال واحد حسب نوعه
function colorizeQuestion(question, index) {
    const { highlight } = question;
    if (!highlight) return;
    
    const element = getQuestionElement(index);
    if (!element) return;
    
    const { color } = highlight;
    
    // Hören / Lesen Teil 2 (نص واحد)
    if (highlight.text) {
        colorizeSimple(element, highlight);
    }
    // Lesen Teil 1 (بداية فقرة + عنوان)
    else if (highlight.paragraphStart && highlight.title) {
        colorizeLesen1(element, highlight);
    }
    // Lesen Teil 3 (بداية نص + بداية موقف)
    else if (highlight.textStart && highlight.situationStart) {
        colorizeLesen3(element, highlight);
    }
    // Sprachbausteine (كلمات متعددة)
    else if (highlight.connector && highlight.highlightWords) {
        colorizeSprach(element, highlight);
    }
}

// تلوين Hören / Lesen Teil 2
function colorizeSimple(element, highlight) {
    const { text, color } = highlight;
    const content = element.innerHTML;
    if (content.includes(text)) {
        element.innerHTML = content.replace(
            text,
            `<span class="highlighted-text" style="background-color:${color};padding:2px 4px;border-radius:3px;font-weight:bold;">${text}</span>`
        );
    }
}

// تلوين Lesen Teil 1
function colorizeLesen1(element, highlight) {
    const { paragraphStart, title, color } = highlight;
    let content = element.innerHTML;
    if (content.includes(paragraphStart)) {
        content = content.replace(
            paragraphStart,
            `<span class="highlighted-text" style="background-color:${color};padding:2px 4px;border-radius:3px;font-weight:bold;">${paragraphStart}</span>`
        );
    }
    if (content.includes(title)) {
        content = content.replace(
            title,
            `<span class="highlighted-text" style="background-color:${color};padding:2px 4px;border-radius:3px;font-weight:bold;">${title}</span>`
        );
    }
...
// تلوين Lesen Teil 3
function colorizeLesen3(element, highlight) {
    const { textStart, situationStart, color } = highlight;
    let content = element.innerHTML;
    if (content.includes(textStart)) {
        content = content.replace(
            textStart,
            `<span class="highlighted-text" style="background-color:${color};padding:2px 4px;border-radius:3px;font-weight:bold;">${textStart}</span>`
        );
    }
    element.innerHTML = content;
    
    const situationElements = document.querySelectorAll('.situation-text, .situation, #leftColumn div');
    situationElements.forEach(el => {
        if (el.textContent && el.textContent.includes(situationStart)) {
            el.innerHTML = el.innerHTML.replace(
                situationStart,
                `<span class="highlighted-text" style="background-color:${color};padding:2px 4px;border-radius:3px;font-weight:bold;">${situationStart}</span>`
            );
        }
    });
}

// تلوين Sprachbausteine
function colorizeSprach(element, highlight) {
    const { highlightWords, color } = highlight;
    let content = element.innerHTML;
    highlightWords.forEach(word => {
        const regex = new RegExp(`\\b${word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'g');
        content = content.replace(
            regex,
            `<span class="highlighted-text" style="background-color:${color};padding:2px 4px;border-radius:3px;font-weight:bold;">${word}</span>`
        );
    });
    element.innerHTML = content;
}
...
function getCurrentExamData() {
    return window.currentExamData || null;
}

function createColoringButton() {
    // البحث عن حاوية الأزرار
    let container = document.querySelector('.exam-controls');
    if (!container) container = document.getElementById('examNavButtons');
    if (!container) {
        // محاولة إنشاء حاوية إذا لم تكن موجودة
        const nav = document.querySelector('nav');
        if (nav) {
            container = document.createElement('div');
            container.className = 'exam-controls';
    ...
    const oldBtn = document.getElementById('coloringToggleBtn');
    if (oldBtn) oldBtn.remove();
    
    const btn = document.createElement('button');
    btn.id = 'coloringToggleBtn';
    btn.textContent = coloringActive ? '🚫 إخفاء التلوين' : '🎨 عرض التلوين';
    btn.style.cssText = `
    ...
    btn.onclick = toggleColoring;
    
    container.appendChild(btn);
}

function setupColoringListener() {
    let lastExamId = window.currentExamId || 1;
    setInterval(() => {
        const currentId = window.currentExamId || 1;
...
window.buildTrueFalseExam = function(container, questions, note) {
    ...
    for (let i = 0; i < questions.length; i++) {
        ...
        radioTrue.onchange = (function(idx) {
            return function() { window._trueFalseUserAnswers[idx] = true; };
        })(i);
        ...
        radioFalse.onchange = (function(idx) {
            return function() { window._trueFalseUserAnswers[idx] = false; };
        })(i);
        ...
        // تطبيق التلوين على النص إذا كان موجوداً
        if (q.highlight && q.highlight.text) {
            const textContent = textSpan.innerHTML;
            if (textContent.includes(q.highlight.text)) {
                textSpan.innerHTML = textContent.replace(
                    q.highlight.text,
                    `<span class="highlighted-text" style="background-color:${q.highlight.color};padding:2px 4px;border-radius:3px;font-weight:bold;">${q.highlight.text}</span>`
                );
            }
        }
...
function renderMatchingQuestions() {
    ...
    for (let i = 0; i < questions.length; i++) {
        ...
        // تطبيق التلوين
        if (q.highlight && q.highlight.paragraphStart) {
            const textContent = questionText.innerHTML;
            if (textContent.includes(q.highlight.paragraphStart)) {
                questionText.innerHTML = textContent.replace(
                    q.highlight.paragraphStart,
                    `<span class="highlighted-text" style="background-color:${q.highlight.color};padding:2px 4px;border-radius:3px;font-weight:bold;">${q.highlight.paragraphStart}</span>`
                );
            }
            if (textContent.includes(q.highlight.title)) {
                questionText.innerHTML = questionText.innerHTML.replace(
                    q.highlight.title,
                    `<span class="highlighted-text" style="background-color:${q.highlight.color};padding:2px 4px;border-radius:3px;font-weight:bold;">${q.highlight.title}</span>`
                );
            }
        }
...
function updateTeil3SelectOptions() {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        for (let s = 0; s < situations.length; s++) {
            if (usedSituations.has(s) && currentAnswer !== s) continue;
            const option = document.createElement("option");
            option.value = s;
...
function renderTeil3Exam() {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        // تطبيق التلوين
        if (item.highlight && item.highlight.textStart) {
            const textContent = itemText.innerHTML;
            if (textContent.includes(item.highlight.textStart)) {
                itemText.innerHTML = textContent.replace(
                    item.highlight.textStart,
                    `<span class="highlighted-text" style="background-color:${item.highlight.color};padding:2px 4px;border-radius:3px;font-weight:bold;">${item.highlight.textStart}</span>`
                );
            }
        }
    ...
    for (let i = 0; i < situations.length; i++) {
        ...
        // تطبيق التلوين
        const sitText = sitDiv.innerHTML;
        for (let j = 0; j < items.length; j++) {
            const item = items[j];
            if (item.highlight && item.highlight.situationStart) {
                if (sitText.includes(item.highlight.situationStart)) {
                    sitDiv.innerHTML = sitText.replace(
                        item.highlight.situationStart,
                        `<span class="highlighted-text" style="background-color:${item.highlight.color};padding:2px 4px;border-radius:3px;font-weight:bold;">${item.highlight.situationStart}</span>`
                    );
                    break;
                }
            }
